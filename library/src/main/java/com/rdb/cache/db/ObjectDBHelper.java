...
public class ObjectDBHelper extends SQLiteOpenHelper {
    ...
    private void initTable(SQLiteDatabase db, String table) {
        ...
        if (OBJECT_TABLE.equals(table)) {
            String sql = "CREATE TABLE IF NOT EXISTS " + OBJECT_TABLE + " ("
                    + ObjectColumn.object_key.name() + " NVARCHAR NOT NULL, "
                    + ObjectColumn.object_value.name() + " TEXT NOT NULL, "
                    + ObjectColumn.object_type.name() + " NVARCHAR NOT NULL, "
                    + ObjectColumn.object_time.name() + " LONG NOT NULL, "
                    + ObjectColumn.object_deadline.name() + " LONG NOT NULL, PRIMARY KEY ("
                    + ObjectColumn.object_key.name() + ", "
                    + ObjectColumn.object_type.name() + "))";
            db.execSQL(sql);
        }
    }
    ...
    private void init() {
        ...
        db.delete(OBJECT_TABLE, ObjectColumn.object_deadline.name() + " < ?",
                new String[]{
                        time
                });
        closeDatabase();
    ...
    public final boolean putString(String key, String value, String type, long deadline) {
        ...
        SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
        ...
        return st.replace(OBJECT_TABLE, contentValues) > 0;
    }
    ...
    public final boolean delete(String key, String type) {
        if (key == null || type == null) {
            ...
            SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
            int delete = st.deleteByCondition(OBJECT_TABLE, ObjectColumn.object_key.name()
                    + " = ? and " + ObjectColumn.object_type.name() + " = ?", new String[]{
                    key, type
            });
            return delete > 0;
        }
    ...
    public final boolean deleteAll() {
        SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
        int delete = st
                .deleteByCondition(OBJECT_TABLE, "1 = 1", new String[]{
                });
        return delete > 0;
    }
    ...
    public final List<String> getStrings(String type, ObjectColumn orderby, boolean asc) {
        if (type == null) {
            ...
            SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
            return st.queryForList(new SQLiteTemplate.RowMapper<String>() {
                @Override
                public String mapRow(Cursor cursor, int index) {
                    return cursor.getString(cursor.getColumnIndex(ObjectColumn.object_value.name()));
                }
            }, OBJECT_TABLE, ObjectColumn.object_type.name()
                    + " = ?", new String[]{
                    type
            }, orderby == null ? null : (orderby.name() + " " + (asc ? "asc"
                    : "desc")));
        }
    ...
    public final boolean updateDeadLine(String key, String type, long deadline) {
        ...
        return st.update(OBJECT_TABLE, contentValues, ObjectColumn.object_key.name()
                + " = ? and " + ObjectColumn.object_type.name() + " = ?", new String[]{
                key, type
        }) > 0;
    }
    ...
    public final <T> ObjectResult<T> getObjectResult(String key, String type, final Class<T> t) {
        ...
        if (key == null || type == null || t == null) {
            ...
            SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
            objectResult = st.queryForObject(new SQLiteTemplate.RowMapper<ObjectResult<T>>() {
                @Override
                public ObjectResult<T> mapRow(Cursor cursor, int index) {
                    ...
                    + " = ? and "
                    + ObjectColumn.object_type.name() + " = ?", new String[]{
                    key, type
            });
        }
        return objectResult;
    ...
    public final <T> ObjectListResult<T> getObjectListResult(String key, String type, final Class<T> t) {
        ...
        if (key == null || type == null || t == null) {
            ...
            SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
            objectListResult = st.queryForObject(new SQLiteTemplate.RowMapper<ObjectListResult<T>>() {
                @Override
    ...
    public final <T> List<ObjectResult<T>> getObjectResults(String type, final Class<T> t,
                                                            ObjectColumn orderby, boolean asc) {
        if (type == null || t == null) {
            Log.e(TAG, "getObjectResults error : type == null || t == null");
        } else {
            SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
            return st.queryForList(new SQLiteTemplate.RowMapper<ObjectResult<T>>() {
                @Override
                public ObjectResult<T> mapRow(Cursor cursor, int index) {
                    return getObjectResult(cursor, t);
                }
    ...
    public final <T> List<ObjectListResult<T>> getObjectListResults(String type, final Class<T> t,
                                                                    ObjectColumn orderby, boolean asc) {
        if (type == null || t == null) {
            Log.e(TAG, "getObjectListResults error : type == null || t == null");
        } else {
            SQLiteTemplate st = SQLiteTemplate.getInstance(this, false);
            return st.queryForList(new SQLiteTemplate.RowMapper<ObjectListResult<T>>() {
                @Override
                public ObjectListResult<T> mapRow(Cursor cursor, int index) {
                    return getObjectListResult(cursor, t);
                }
    ...
    public final <T> List<T> getObjects(String type, Class<T> t, ObjectColumn orderby,
                                        boolean asc) {
    ...
    public final <T> List<T> getObjects(String type, Type typeOfT, ObjectColumn orderby,
                                        boolean asc) {
        List<T> list = new ArrayList<T>();
        List<String> values = getStrings(type, orderby, asc);
        for (String value : values) {
